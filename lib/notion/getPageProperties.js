...
async function getPageProperties(id, pageBlock, schema, authToken) {
  ...
  // first get the PageCover, otherwise get the content image
  function getPostCover(pageBlock) {
    ...
    if (pageCover && pageCover.startsWith('/')) {
      ...
      return defaultMapImageUrl(pageCover, pageBlock)
    } else {
      return BLOG?.defaultCover
    }
  }

  properties.page_cover = getPostCover(pageBlock)
