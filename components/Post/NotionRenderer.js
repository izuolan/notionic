...
const components = {
  // Code block
  Code: dynamic(async () => {
    const notionModule = await import('react-notion-x/build/third-party/code')
    ...
    return notionModule.Code
  }),
  // Database block
  Collection: dynamic(async () => {
    const notionModule = await import('react-notion-x/build/third-party/collection')
    return notionModule.Collection
  }),
  // Equation block & inline variant
  Equation: dynamic(async () => {
    const notionModule = await import('react-notion-x/build/third-party/equation')
    return notionModule.Equation
  })
}
